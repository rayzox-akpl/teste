...
-- avant chaque lancement, ce qui laissait passer une 2e instance.
-- Celle-ci n'est jamais reinitialisee de l'exterieur.
...
local SAFE_HEIGHT = 400      -- hauteur de repli par defaut
local HEAL_HEIGHT = 350      -- soin : assez bas pour garder le boss charge
local DODGE_HEIGHT = 400     -- esquive : au-dessus de la zone du Decay
...
--====================================================
-- MONTEE EN SECURITE (verrou partage)
-- Empeche le cumul : dodge + heal ne font pas +4000.
--====================================================

-- Verrou GLOBAL : partage entre toutes les instances du script.
-- Si deux copies tournent en meme temps, l'altitude ne se cumule pas.
_G.KronaxSafe = _G.KronaxSafe or { elevated = false, ground = nil }
local Safe = _G.KronaxSafe

-- Monte au-dessus du sol, une seule fois.
-- height : hauteur voulue, SAFE_HEIGHT par defaut.
local function goSafe(height)
    if Safe.elevated then
        -- Deja en hauteur : on ne rajoute rien
        startFloat()
        return
    end

    Safe.elevated = true
    Safe.ground = hrp.CFrame

    hrp.CFrame = Safe.ground + Vector3.new(0, height or SAFE_HEIGHT, 0)
    stabilize()
    startFloat()
end

-- Redescend sur la position d'orbite courante, puis libere le verrou
local RETURN_WAIT = 6   -- attente max d'une cible avant de redescendre

local function pickTarget()
    if focusEnabled then
        return lockedCrystalPart or findMob()
    end
    return bossPart or findBossFallback()
end

local function returnFromSafe()
    if not Safe.elevated then return end

    -- On garde le float tant qu'aucune cible n'existe : sans ca
    -- le perso tombe des 2000 studs pendant la recherche.
    local backTarget = pickTarget()
    local deadline = os.clock() + RETURN_WAIT

    while (not backTarget or not backTarget.Parent) and os.clock() < deadline do
        startFloat()
        task.wait(0.2)
        backTarget = pickTarget()
    end

    if backTarget and backTarget.Parent then
        local bx = math.cos(angle) * DISTANCE_MOB
        local bz = math.sin(angle) * DISTANCE_MOB
        hrp.CFrame = CFrame.new(
            Vector3.new(
                backTarget.Position.X + bx,
                backTarget.Position.Y + HEIGHT,
                backTarget.Position.Z + bz
            ),
            backTarget.Position
        )
    elseif Safe.ground then
        hrp.CFrame = Safe.ground
    end
...
local function startSpamLoop()
    ...
    -- Ne redescend que si un dodge ne nous garde pas en haut
...
-- Anti-rebond : DescendantAdded se declenche pour chaque descendant.
-- Un meme effet peut donc lever l'evenement plusieurs fois.
local lastDodge = 0
local DODGE_DEBOUNCE = 2

workspace.DescendantAdded:Connect(function(obj)
    if obj.Name ~= "TDecayCharge" then return end
    if attackInProgress or securityMode then return end
    if os.clock() - lastDodge < DODGE_DEBOUNCE then return end
    ...
    task.delay(DODGE_TIME, function()
        -- Ne redescend que si le heal ne nous garde pas en haut
        if not securityMode then
            returnFromSafe()
        end
