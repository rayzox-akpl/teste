...
local isElevated = false
local groundCFrame = nil

-- Monte a SAFE_HEIGHT au-dessus du sol, une seule fois
local function goSafe()
    ...
    isElevated = true
    groundCFrame = hrp.CFrame

    hrp.CFrame = groundCFrame + Vector3.new(0, SAFE_HEIGHT, 0)
    stabilize()
    startFloat()
end
...
local function returnFromSafe()
    if not isElevated then return end

    local backTarget = nil
    if focusEnabled then
        backTarget = lockedCrystalPart or findMob()
    else
        backTarget = bossPart or findBossFallback()
    end
